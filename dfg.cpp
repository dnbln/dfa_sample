...
static void build_dfg_nodes(std::shared_ptr<CfgNode> cfg_node, Dfg &dfg) {
...
static void forward_link_dfg_nodes(std::shared_ptr<CfgNode> cfg_node, Dfg &dfg) {
    std::visit(
            [&](auto &&node) {
                using T = std::decay_t<decltype(node)>;
                if constexpr (std::is_same_v<T, BasicCfgBlock>) {
                    std::shared_ptr<DfgNode> dfg_node = dfg_ptr_for_cfg(dfg, *cfg_node);
                    ...
                    std::shared_ptr<DfgNode> dfg_node = dfg_ptr_for_cfg(dfg, *cfg_node);
                    std::optional<std::weak_ptr<CfgNode>> while_node = node->while_node;
                    std::shared_ptr<CfgNode> while_node_locked = while_node.value().lock();
...
static void backward_link_dfg_nodes(Dfg& dfg, std::shared_ptr<DfgNode> node, std::optional<std::weak_ptr<DfgNode>> prev) {
    if (prev.has_value()) {
        node->in_nodes.push_back(prev.value());
    }

    for (const auto& out_node: node->out_nodes) {
        std::shared_ptr<DfgNode> out_node_locked = out_node.lock();
        bool skip = false;
        ...
        if (skip) continue;
        backward_link_dfg_nodes(dfg, out_node_locked, std::make_optional(node));
    }
}
